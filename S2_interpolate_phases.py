...
def interpolation(surface_path, ieeg_data_array, coordinates, start_sample, end_sample, save_path, ifsave):
    ...
    参数:
        ...
        ieeg_data_array: 电极相位数据, shape=(时间, 电极数)
    ...
    # 取插值结果的相位
    interpolated_values = np.angle(interpolated_values)

    # 保存插值结果
    if ifsave is True:
        pd.DataFrame(interpolated_values).to_csv(save_path)    # (t, 点数)

def compute_phase_from_potentials(potentials):
    '''
    计算每个电极的瞬时相位
    参数:
        potentials: 电极电势数据, shape=(时间, 电极数)
    返回:
        phases: 相位数据, shape=(时间, 电极数)
    '''
...
if __name__ == "__main__":
    ...
    for run_num in run_numbers:
        ...
        for trial_name in trials_folder_names:
            ...
            save_path = f"{save_folder}/{subfolder}_{ses}_task-SPESclin_run-{run_num}-{trial_name}-ave-interpolation_phases_data.csv"

            # 计算相位
            phases = compute_phase_from_potentials(ieeg_data)
            print("phases nan: ", np.sum(np.isnan(phases)))
            ...
            # 执行插值
            interpolation(surface_path, phases, coordinates, start_sample, end_sample, save_path, ifsave)