...
"""
This code visualizes the velocity field calculated using the optical flow method. Key steps include:
- Load calculated velocity fields, orthogonal bases and surface.
- Process the velocity field data and convert it into coordinate-based velocity vectors.
- Use the PyVista library to draw velocity fields and singular points on surfaces, either as static plots or as animated GIFs.
This code allows the user to choose to display the original velocity field, or just the velocity field direction (normalized velocity vector, fixed length for easier viewing)
"""
...
def plot_velocity_fields_and_singularity_points_gif(surface, potentials, V_k_coord, singularity_points, velocity_fields_gif_path):
    ...
    surface['V_scale'] = np.ones((len(V_k_coord[0]), )) * 10
...
# 在曲面上绘制速度场和临界点的动态 GIF 图像
def plot_velocity_fields_and_singularity_points_and_time_gif(sfreq, begin, surface, potentials, V_k_coord, singularity_points, velocity_fields_gif_path):
    """
    在曲面上绘制速度场和临界点的动态 GIF 图像
    ...
    actor1 = p.add_mesh(arrows, color='black')
    actor2 = p.add_mesh(pv.PolyData(singularity_points[0]), color='red', point_size=10)
    actor3 = p.add_text(str(begin), font_size=15)
    p.add_mesh(surface, scalars="Potentials", cmap="terrain", clim=[cmin, cmax], show_edges=False, smooth_shading=False)
    ...
    for i in range(len(V_k_coord)):
        ...
        actor1                = p.add_mesh(arrows, color='black')
        actor2                = p.add_mesh(pv.PolyData(singularity_points[i]), color='red', point_size=10)
        actor3                = p.add_text(str(begin + i / sfreq), font_size=15)
        p.add_mesh(surface, scalars="Potentials", cmap="terrain", clim=[cmin, cmax], show_edges=False, smooth_shading=False)
        p.write_frame()
...
if "__main__" == __name__:
    with open("Code/opticalflow.yaml", 'r', encoding='UTF-8') as file:
    ...
    plot_velocity_fields_and_singularity_points_and_time_gif(512, 0.009, surface, potentials, V_k_coord, singularity_points, velocity_fields_gif_path)
